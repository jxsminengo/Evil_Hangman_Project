...
GENERIC_VECTOR avl_tree_get_largest_family(TREE hMy_tree, int value) {
    AVL_tree *pMy_tree = (AVL_tree*)hMy_tree;
    GENERIC_VECTOR return_vector = NULL;
    GENERIC_VECTOR temp;
    int i;

    temp = find_max_family(pMy_tree->root, value);
...
GENERIC_VECTOR find_max_family(Node *root, int value) {
    Generic_vector *temp = NULL;

    if (root != NULL) {
      if (root->left != NULL) {
        temp = find_max_family(root->left, value);
      ...
      if (root->right != NULL) {
        temp = find_max_family(root->right, value);
        if (temp == NULL || generic_vector_get_size(temp) < generic_vector_get_size(root->data)) {
          temp = root->data;
        }
      }
